...
const val LOG_TAG = "dl9rdz-maptest"
...
class AprsHandler {
    ...
    fun run() {
	Log.d(LOG_TAG, "APRS thread is running")
	while(true) {
	...
	*/
	...
	jloop@ while(true) {
	    ...
	    when(byte) {
		-1 ->  break@jloop
		'}'.toInt() -> { buf += byte.toByte(); processFrame(buf); buf = byteArrayOf() }
		'\n'.toInt() -> {}
		else -> buf += byte.toByte()
	    }
	}
...
class RdzWx : CordovaPlugin() {
    ...
    val aprsHandler = AprsHandler()

    val runnable: Runnable = run {
        Runnable {
            Log.d(LOG_TAG, "Runnable is running - test")
	    aprsHandler.postAlive()
            handler.postDelayed(runnable, 5000)
        }
    }

    fun runAprs(serviceInfo: NsdServiceInfo) {
	Log.d(LOG_TAG, "setting target host for aprs handler")
	aprsHandler.connectTo(serviceInfo.host, serviceInfo.port)
    }

    fun updateGps(latitude: Double, longitude: Double, altitude: Double, bearing: Float) {
	aprsHandler.postGpsPosition(latitude, longitude, altitude, bearing)
    }
